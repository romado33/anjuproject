# Application walkthrough — Anju Internal Case Router
...
| **Case intake**     | `app/pages/1_Case_intake.py`  | Run sample scenarios, optional custom text |
| **Review case actions** | `app/pages/2_Review_case_actions.py` | Approvals, mock execute, audit export |
| **Integrations** | `app/pages/4_Integrations.py` | Adapter base URLs and parameters (persisted JSON; mock execution unchanged) |
...
- Summarize the one-line intro under the title (AI routing, human approvals, automated execution).
...
- **Four sample scenarios** (2×2): typical Anju paths — IRMS implementation, FDA/TrialMaster compliance, TrialMaster export, TA Scan feasibility. Each primary button runs the full pipeline and lands in `pending_approval` when complete.
...
- Open **Integrations** in the sidebar: per-adapter **base URL**, **enabled**, **parameters** (JSON string map), and notes.
- Saves to `data/integration_settings.json` (or `INTEGRATION_SETTINGS_PATH`). The app still uses **mock** adapter responses; production wiring would read this file from `Settings`.
